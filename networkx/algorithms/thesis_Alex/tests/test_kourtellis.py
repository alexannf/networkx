...
class TestKourtellisBetweenness:
    ...
    def test_construct_facebook_0(self):
        ...
        G_dyn = nx.parse_edgelist(list(G.edges)[None:1500:None])
        G_bc = nx.parse_edgelist(list(G.edges)[None:1500:None])

        bc2, D2, SP2, Delta2 = \
            nx.betweenness_centrality(G_dyn, weight=None, normalized=False, xtra_data=True)

        for edge in list(G.edges)[1500:None:None]:
